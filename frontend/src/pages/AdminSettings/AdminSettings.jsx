...
function AdminSettings() {
  ...
  const [modifiedSetting, setModifiedSetting] = useState({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setSettings(json.mockedSettings2);
      } catch (err) {
        console.error(err);
      }
    };
  ...
  function changeSetting(name, value) {
    ...
    setModifiedSetting((prev) => ({ ...prev, [name]: value }));
  }
          ...
          {Object.entries(settings).map(([name, value]) => (
            <div key={name} className="grid grid-cols-3 text-center">
              ...
              <input
                type="button"
                value="X"
                onClick={(e) => deleteSetting(name, e.target.value)}
                className="col-span-1 border border-gray-400 bg-red-800 text-white hover:bg-red-600"
              ></input>
            </div>
          ))}
          <h3 className="text-lg font-bold mb-4">Créer nouveau setting:</h3>
