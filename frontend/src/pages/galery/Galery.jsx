...
const PlayIcon = ({ size = 5, className = "" }) => (
  <svg className={`w-${size} h-${size} ${className}`} fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
);
...
useEffect(() => {
  ...
  const sortOptions = [
    ...
    { id: "title-desc", name: "Titre Z-A" },
    { id: "duration", name: "Durée ↑" },
    { id: "duration-desc", name: "Durée ↓" },
    { id: "year", name: "Année" },
    { id: "country", name: "Pays" },
  ];

  const stats = [1, 2, 3, 4].map(i => ({ num: t(`gallery.gallery_stat_${i}`), label: t(`gallery.gallery_stat_${i}_label`) }));
  ...
  const films = FILMS
    .filter(f => {
      ...
      return (category === "all" || f.category === category) && 
             (!search || [f.title, f.director, f.country, f.description].some(s => s.toLowerCase().includes(q)));
    })
    .sort((a, b) => {
      switch(sortBy) {
      ...
      <div className="w-full overflow-hidden bg-[#050508] min-h-screen">
        ...
        <section className="relative py-32 px-10 bg-gradient-to-br from-[#0a0a0f] via-[#1a0a2e] to-[#050508] overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto relative z-5 text-center">
            <div className="text-[#06b6d4] text-xs tracking-[3px] uppercase mb-6 font-inter">{t("gallery.gallery_badge")}</div>
            <h1 className="font-orbitron font-black text-[clamp(48px,10vw,120px)] leading-[0.9] mb-6">
              <span className="bg-gradient-to-br from-white to-[#e0e0ff] bg-clip-text text-transparent">{t("gallery.gallery_title")}</span><br />
              <span className="bg-gradient-to-br from-[#a855f7] to-[#ec4899] bg-clip-text text-transparent drop-shadow-[0_0_40px_rgba(236,72,153,0.8)]">{t("gallery.gallery_title_highlight")}</span>
            </h1>
            <p className="text-lg text-[#a0a0b8] max-w-2xl mx-auto">{t("gallery.gallery_description")}</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-16 max-w-4xl mx-auto">
              {stats.map((s, i) => (
                <div key={i} className="text-center p-6 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm">
                  <div className="font-orbitron font-black text-3xl text-[#ec4899] mb-2">{s.num}</div>
                  <div className="text-xs text-[#a0a0b8] tracking-wider uppercase">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-10">
            <div className="max-w-2xl mx-auto mb-8 relative">
              ...
              <input
                ...
                placeholder={t("gallery.gallery_search_placeholder") || "Rechercher..."}
              ...
              />
              {search && (
                <button onClick={() => setSearch("")} className="absolute right-5 top-1/2 -translate-y-1/2 text-[#6b6b85] hover:text-white">
            ...
            <div className="flex flex-wrap gap-4 justify-center items-center">
              ...
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
                ...
                {sortOptions.map(s => <option key={s.id} value={s.id} className="bg-[#0a0a0f]">{s.name}</option>)}
              </select>
            </div>
            <p className="text-center mt-6 text-sm text-[#6b6b85] font-inter">{films.length} {t("gallery.gallery_films_found")}</p>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto px-10">
              ...
              <div className="flex flex-col lg:flex-row gap-8 items-center mb-12">
                <Link to={`/video/${film.id}`} className="w-full lg:w-2/3 group">
                  <div className="relative aspect-video rounded-3xl overflow-hidden bg-[#1a1a24] shadow-[0_20px_60px_rgba(168,85,247,0.3)]">
                    ...
                    <div className="absolute top-4 right-4 px-4 py-2 bg-black/60 backdrop-blur-sm rounded-full text-white text-sm font-inter font-semibold">{film.duration}</div>
                    <div className="absolute top-4 left-4 px-4 py-2 bg-[#a855f7]/80 backdrop-blur-sm rounded-full text-white text-sm font-inter font-semibold uppercase">{categories.find(c => c.id === film.category)?.name.split(" ")[0]}</div>
                  </div>
                </Link>
                <div className="w-full lg:w-1/3 space-y-6">
                  <span className="text-[#a855f7] text-sm font-inter font-semibold uppercase tracking-wider">Film sélectionné • {filmIndex + 1}/{films.length}</span>
                  <h2 className="font-orbitron font-black text-4xl lg:text-5xl text-white leading-tight">{film.title}</h2>
                  <div className="flex flex-wrap items-center gap-4 text-[#a0a0b8]">
                    <span className="flex items-center gap-2">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                      {film.director}
                    </span>
                    <span>•</span><span>{film.country}</span><span>•</span><span>{film.year}</span>
                  </div>
                  <p className="text-[#a0a0b8] text-lg leading-relaxed">{film.description}</p>
                  <Link to={`/video/${film.id}`} className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-[#a855f7] to-[#ec4899] text-white font-inter font-semibold text-sm tracking-wider uppercase rounded-full hover:shadow-[0_15px_40px_rgba(168,85,247,0.5)] transition-all">
                    <PlayIcon /> Regarder
                  </Link>
                </div>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-orbitron font-bold text-xl text-white">Tous les films ({films.length})</h3>
                <div className="flex gap-2">
                  {['left', 'right'].map(dir => (
                    <button key={dir} onClick={() => scroll(dir)} className="w-10 h-10 rounded-full bg-white/10 hover:bg-[#a855f7] flex items-center justify-center text-white transition-all">
              ...
              <div ref={sliderRef} className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide scroll-smooth" style={{ scrollbarWidth: 'none' }}>
                {loading ? (
                  ...
                  visibleFilms.map((f, i) => (
                    <div key={f.id} onClick={() => setFilmIndex(i)} className={`flex-shrink-0 w-64 cursor-pointer transition-all ${filmIndex === i ? 'scale-105 opacity-100' : 'opacity-60 hover:opacity-100'}`}>
                      <div className={`relative aspect-video rounded-2xl overflow-hidden mb-3 ${filmIndex === i ? 'ring-4 ring-[#a855f7] shadow-[0_0_30px_rgba(168,85,247,0.5)]' : 'hover:ring-2 hover:ring-white/30'}`}>
                        ...
                        <div className="absolute bottom-2 right-2 px-2 py-1 bg-black/70 rounded text-white text-xs">{f.duration}</div>
                      </div>
                      <h4 className={`font-orbitron font-bold text-sm truncate ${filmIndex === i ? 'text-[#a855f7]' : 'text-white'}`}>{f.title}</h4>
                      <p className="text-xs text-[#6b6b85] truncate">{f.director}</p>
                    </div>
                  ))
                )}
              </div>
              ...
              {visibleCount < films.length && (
                <div ref={loaderRef} className="flex justify-center py-8">
                  <div className="flex gap-2">
                    {[...Array(3)].map((_, i) => (
                      <div key={i} className="w-3 h-3 rounded-full bg-[#a855f7] animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
                    ))}
                  </div>
                </div>
              )}
            ...
            {films.length === 0 && (
              <div className="text-center py-20">
                ...
                <h3 className="font-orbitron font-bold text-2xl text-[#a0a0b8] mb-4">{t("gallery.gallery_empty_title")}</h3>
                <p className="text-[#6b6b85]">{t("gallery.gallery_empty_desc")}</p>
              </div>
            )}
          </div>
        </section>
