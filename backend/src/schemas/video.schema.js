...
const VideoSchema = videosSchema
  ...
  .extend({
    ...
    cover_image: fileSchema(5 * 1024 * 1024, [
      "image/jpeg",
      "image/png",
      "image/webp",
      "image/gif",
    ...
    producer_image: fileSchema(5 * 1024 * 1024, [
      "image/jpeg",
      "image/png",
      "image/webp",
      "image/gif",
  ...
  schema.superRefine((data, ctx) => {
    ...
    if (!metadata) {
      ctx.addIssue({
        code: "custom",
        path: ["filename"],
        message: "Could not parse video metadata",
      });
      return;
    }
    if (metadata.duration > 90) {
      ctx.addIssue({
        code: "custom",
        path: ["filename"],
        message: "Video duration must not exceed 90 seconds",
      });
    }
