...
import { createAdmin, findAdminByLogin, findAdminById, addPasswordAdmin } from '../models/admin.model.js';
import { createToken, findToken, updateTokenStatus } from '../models/token.model.js';
...
export const inviteAdmin = async (req, res) => {
	try {
		...
		console.log(`[EMAIL MOCK] To: ${login}, Subject: Admin Invite, Body: Cliquer ici pour définir votre mot de passe: ${inviteLink}`);

		res.status(201).json({ message: 'Invite sent successfully', adminId: adminId.toString() });
...
export const verifyInvite = async (req, res) => {
	...
	try {
		...
		res.status(error.status || 500).json({ valid: false, message: error.message || 'Server error' });
	}
};
...
export const acceptInvite = async (req, res) => {
	...
	try {
		...
		return res.status(200).json({ valid: true, message: 'Token validated and password set' });
